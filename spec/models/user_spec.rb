...
RSpec.describe User, type: :model do
    ...
    it 'does not save user without level' do
        user = build(:user, level: nil)
        expect { user.save! }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'does not save user with invalid level' do
        user = build(:user, level: 'invalid')
        expect { user.save! }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'does not save user with email longer than 255 characters' do
        user = build(:user, email: 'a' * 256)
        expect { user.save! }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'does not save user with first name longer than 255 characters' do
        user = build(:user, first_name: 'a' * 256)
        expect { user.save! }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'does not save user with last name longer than 255 characters' do
        user = build(:user, last_name: 'a' * 256)
        expect { user.save! }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'does not save user with password longer than 255 characters' do
        user = build(:user, password: 'a' * 256)
        expect { user.save! }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'saves user with email, password, first name, and last name' do
        user = build(:user)
    ...
    it 'queries all reviews for a user' do
        user = create(:user)
    ...
    it 'queries all contracts a user is the point of contact for' do
        user = create(:user)
